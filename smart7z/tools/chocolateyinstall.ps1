...
Copy-Item "$(Join-Path $extractLocation '*.fav')" "$($installLocation)" -Recurse -Force

Write-Output "Install completed"

#block shimgen
$exes = Get-ChildItem -Recurse $extractLocation -Include "*.exe" -Name
ForEach($exe in $exes) {
       New-Item "$(Join-Path $extractLocation ($exe + ".ignore"))" -ItemType File -Force | Out-Null
}
